...
bool Font::Load(const std::string& fileName)
{
	...
	for (auto size : fontSizes) // NOLINT
	{
		TTF_Font* font = TTF_OpenFont(fileName.c_str(), static_cast<float>(size));
		if (font == nullptr)
		{
...
Texture* Font::RenderText(const std::string& text, const Vector3& color /*= Color::White*/,
						  int pointSize /*= 24*/, int wrapLength /*= 900*/)
{
	...
	if (iter != mFontData.end())
	{
		...
		SDL_Surface* surf = TTF_RenderText_Blended_Wrapped(font, text.c_str(), 0, sdlColor,
														   wrapLength);
		if (surf != nullptr)
		{
