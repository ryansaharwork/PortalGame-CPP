...
public:
	...
	void StopPlayback();
	bool IsInPlayback() const { return mIsInPlayback; }

	void RecordInput(const bool* keyState, Uint32 mouseButtons, const Vector2& relativeMouse);
	void Update(float deltaTime);

	void InputPlayback(const bool*& keyState, Uint32& mouseButtons, Vector2& relativeMouse);
...
private:
	...
	class Actor* GetBluePortal() const;
	class Actor* GetOrangePortal() const;
	...
	static void FillPortalInfo(class Actor* portal, PortalInfo& outInfo);
	static void WritePortalJSON(const PortalInfo& info, rapidjson::MemoryPoolAllocator<>& allocator,
								rapidjson::Value& outJSON);
	static void ReadPortalJSON(const rapidjson::Value& value, PortalInfo& outInfo);
	...
	bool mPlaybackKeys[SDL_SCANCODE_COUNT];
