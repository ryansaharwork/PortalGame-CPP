...
namespace
{
	...
	void ValidateQuat(const char* description, const Quaternion& expected, const Quaternion& actual)
	{
		...
		{
			SDL_LogWarn(0, "%s mismatch.\nExpected: (%f, %f, %f, %f)\nActual:   (%f, %f, %f, %f)",
						description, expected.GetX(), expected.GetY(), expected.GetZ(),
						expected.GetW(), actual.GetX(), actual.GetY(), actual.GetZ(),
						actual.GetW());
			SDL_assert(
				false &&
				"InputReplay validation failed, check output log for details or break for debugger");
...
: mGame(game)
{
	...
	std::memset(mPlaybackKeys, 0, sizeof(bool) * SDL_SCANCODE_COUNT);
}
...
void InputReplay::RecordInput(const bool* keyState, Uint32 mouseButtons,
...
void InputReplay::InputPlayback(const bool*& keyState, Uint32& mouseButtons, Vector2& relativeMouse)
{
	if (mIsInPlayback)
	{
			...
			if (mBluePortalInfo.mExists)
			{
				...
				else
				{
					ValidateVector("Blue portal position", mBluePortalInfo.mPosition,
								   mBluePortal->GetTransform().GetPosition());
					ValidateQuat("Blue portal quat", mBluePortalInfo.mQuat,
								 mBluePortal->GetTransform().GetQuat());
					...
					if (cc)
					{
						Vector3 size = cc->GetSize();
						width = size.y;
						height = size.z;
						depth = size.x;
					}
					ValidateFloat("Blue portal collision.x", mBluePortalInfo.mDepth, depth);
					ValidateFloat("Blue portal collision.y", mBluePortalInfo.mWidth, width);
					ValidateFloat("Blue portal collision.z", mBluePortalInfo.mHeight, height);
				}
			}
			...
			if (mOrangePortalInfo.mExists)
			{
				...
				else
				{
					ValidateVector("Orange portal position", mOrangePortalInfo.mPosition,
								   mOrangePortal->GetTransform().GetPosition());
					ValidateQuat("Orange portal quat", mOrangePortalInfo.mQuat,
								 mOrangePortal->GetTransform().GetQuat());
					...
					ValidateFloat("Orange portal collision.x", mOrangePortalInfo.mDepth, depth);
					ValidateFloat("Orange portal collision.y", mOrangePortalInfo.mWidth, width);
					ValidateFloat("Orange portal collision.z", mOrangePortalInfo.mHeight, height);
				}
			}
...
void InputReplay::FillPortalInfo(class Actor* portal, PortalInfo& outInfo)
{
	...
	if (portal)
	{
		outInfo.mPosition = portal->GetTransform().GetPosition();
		outInfo.mQuat = portal->GetTransform().GetQuat();
		CollisionComponent* cc = portal->GetComponent<CollisionComponent>();
		if (cc)
		{
			Vector3 size = cc->GetSize();
			outInfo.mWidth = size.y;
			outInfo.mHeight = size.z;
			outInfo.mDepth = size.x;
		}
	}
}
								  ...
								  rapidjson::Value& outJSON)
{
	...
	else
	{
		...
		portalQuat.SetArray();
		portalQuat.PushBack(info.mQuat.GetX(), allocator);
		portalQuat.PushBack(info.mQuat.GetY(), allocator);
		portalQuat.PushBack(info.mQuat.GetZ(), allocator);
		portalQuat.PushBack(info.mQuat.GetW(), allocator);
...
void InputReplay::ReadPortalJSON(const rapidjson::Value& value, PortalInfo& outInfo)
{
	...
	if (outInfo.mExists)
	{
		...
		outInfo.mQuat.Set(value["q"][0].GetFloat(), value["q"][1].GetFloat(),
						  value["q"][2].GetFloat(), value["q"][3].GetFloat());
...
void InputReplay::ResetPlayer() const
{
	// Reset initial player position/camera/movement
	if (Player* player = gGame.GetPlayer())
	{
		player->GetTransform().SetPosition(player->GetInitialPos());
		player->GetTransform().SetRotation(0.0f);

		if (CameraComponent* cam = player->GetComponent<CameraComponent>())
		{
			cam->ResetPitch();
		}

		if (PlayerMove* move = player->GetComponent<PlayerMove>())
		{
			move->ResetMove();
		}
	}
...
Vector3 InputReplay::GetPlayerPosition() const
{
	if (Player* p = gGame.GetPlayer())
	{
...
const Vector3& InputReplay::GetPlayerVelocity() const
{
	...
	if (Player* p = gGame.GetPlayer())
	{
		if (auto* mv = p->GetComponent<PlayerMove>())
		{
...
float InputReplay::GetPlayerYaw() const
{
	if (Player* p = gGame.GetPlayer())
	{
		return p->GetTransform().GetRotation();
	}
	return 0.0f;
}
...
float InputReplay::GetPlayerPitch() const
{
	if (Player* p = gGame.GetPlayer())
	{
		if (auto* cam = p->GetComponent<CameraComponent>())
		{
			return cam->GetPitchAngle();
		}
	}
	return 0.0f;
}

Actor* InputReplay::GetBluePortal() const
{
	return gGame.GetBluePortal();
}

Actor* InputReplay::GetOrangePortal() const
{
	return gGame.GetOrangePortal();
}
